...
Sources outside this central region are attenuated by a primary beam response factor of less that 1 in 20,000 relative to the pointing centre, and so a wider open-Clean region is unnecessary unless this margin happens to contain a source with flux level on the order of one Jansky. 
 ...
 We initialize this \textit{Clean} operation with the source-component model derived from the masked \textit{Clean} process, which should help to ensure that any bright sources in the field are cleaned correctly, without spurious assignment of source-components to bright side-lobe artefacts.
...
The subroutine repeatedly applies the \textit{Clean} algorithm until acceptance criteria are met. 
The convergence tolerance (shown as 5\% in figure), and the \textit{Clean} threshold as a multiple of the estimated root-mean-square background noise level (RMS), are both user-configurable. 
...
\begin{itemize}
   \item The observations of XTEJ908+094 are affected by a very bright out-of-field source \textit{(which?)}, which presents extended side-lobe artefacts as additional background variation in the field-of-view.
